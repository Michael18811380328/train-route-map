...
- [ ] 考虑曲线效果（贝塞尔曲线）路线变成曲线，车辆在曲线上起来：两个车站之间，我们使用贝塞尔曲线进行实现，那么就是一个曲线函数可以绘制。某一个车次在两个车站的时间是固定的，我们假设匀速行驶，那么某个时间可以计算出位置。实际上比较困难，因为曲线的每一个坐标都需要算，碰撞检测等难度增加。——从极限的角度考虑，实际上曲线就是无数直线组成的，这个 demo 简化成直线。
